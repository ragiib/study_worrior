// ============================================================================
// Tasks Screen - Daily Task tracker with calendar view
// Shows Tasks with streak counting and a calendar heat map.
// ============================================================================
...
class _TasksScreenState extends State<TasksScreen> {
  ...
  String? _selectedTaskId; // Track which Task's calendar to show
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    SizedBox(width: 8),
                    _buildToggleChip('All Tasks', !_showOnlyToday, () {
            ...
            Expanded(
              child: Consumer<TaskProvider>(
                builder: (context, taskProvider, _) {
                  var displayTasks = taskProvider.tasks;
                  if (_showOnlyToday) {
                    displayTasks = displayTasks
                        .where((t) => t.isScheduledFor(DateTime.now()))
                        .toList();
                  }
                    ...
                    children: [
                      ...
                      ...displayTasks.map(
                        (task) => _TaskCard(
                          ...
                          onToggle: () {
                            if (task.isScheduledFor(DateTime.now())) {
                              taskProvider.toggleTaskToday(task.id);
                            } else {
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text('This Task is not scheduled for today.'),
                      ...
                      // Calendar view for selected Task
                      if (_selectedTaskId != null) ...[
                        SizedBox(height: 16),
                        _buildCalendarView(context, taskProvider),
                      ],
                    ],
                  );
                },
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 8),
          Text(
            'Start building powerful daily Tasks!',
            style: Theme.of(context).textTheme.bodyMedium,
          ),
        ],
      ),
  ...
  // ── Calendar View (for a selected Task) ────────────────────────────
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Text(task.emoji, style: TextStyle(fontSize: 20)),
              SizedBox(width: 8),
              Text(
                '${task.name} Calendar',
          ...
          TableCalendar(
            ...
            calendarBuilders: CalendarBuilders(
              ...
              defaultBuilder: (context, day, focusedDay) {
                ...
                } else if (!task.isScheduledFor(day)) {
                  return Center(
                    child: Text(
                      '${day.day}',
                      style: TextStyle(
  ...
  void _showTaskDialog(BuildContext context, [Task? task]) {
    ...
    final nameController = TextEditingController(text: isEditing ? task.name : '');
    final descController = TextEditingController(text: isEditing ? task.description : '');
    String selectedEmoji = isEditing ? task.emoji : '📚';
    List<int> selectedDays = isEditing ? List.from(task.scheduledDays) : [1, 2, 3, 4, 5, 6, 7];
...
class _TaskCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final isScheduledToday = task.isScheduledFor(DateTime.now());
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          SizedBox(width: 14),
          // Task details (Tappable to show Calendar)
          Expanded(
